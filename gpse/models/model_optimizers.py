"""
Model hyperparameter optimization modules for both regression and classification tasks.
...
class ModelOptimizer:
    ...
    def __init__(self, random_state: Optional[int] = None, n_threads: int = 1):
        self.random_state = random_state
        self.n_threads = n_threads  # 添加线程数参数
    ...
    # Classification parameter functions
    def _histgradientboost_clf_params(self, trial: optuna.Trial) -> Dict[str, Any]:
        return {
            'max_depth': trial.suggest_int('max_depth', 1, 20),
            'max_iter': trial.suggest_int('max_iter', 10, 500, log=True),
            'max_leaf_nodes': trial.suggest_int('max_leaf_nodes', 2, 100),
    ...
    def _gbdt_clf_params(self, trial: optuna.Trial) -> Dict[str, Any]:
        param_dict = {
            'random_state': self.random_state,
            'n_iter_no_change': 20
        }
    ...
    def _sgd_clf_params(self, trial: optuna.Trial) -> Dict[str, Any]:
        penalty = trial.suggest_categorical('penalty', ['l2', 'l1', 'elasticnet'])
        param_dict = {
        ...
        if penalty == 'elasticnet':
            param_dict['l1_ratio'] = trial.suggest_float('l1_ratio', 0, 1)
        
        return param_dict
    
    def _elasticnet_clf_params(self, trial: optuna.Trial) -> Dict[str, Any]:
        return {
            'C': trial.suggest_float('C', 1e-5, 1e2, log=True),
            'l1_ratios': trial.suggest_float('l1_ratios', 0, 1),
            'random_state': self.random_state
        }
    
    def _bayesianridge_clf_params(self, trial: optuna.Trial) -> Dict[str, Any]:
        return {
            'C': trial.suggest_float('C', 1e-5, 1e2, log=True),
    ...
    def _gradientboost_clf_params(self, trial: optuna.Trial) -> Dict[str, Any]:
        param_dict = {
            'random_state': self.random_state,
            'n_iter_no_change': 20
        }
    ...
    def _knn_clf_params(self, trial: optuna.Trial) -> Dict[str, Any]:
        return {
            'n_neighbors': trial.suggest_int('n_neighbors', 1, 101, step=5),
            'weights': trial.suggest_categorical('weights', ['uniform', 'distance'])
        }
    ...
    def _adaboost_clf_params(self, trial: optuna.Trial) -> Dict[str, Any]:
        return {
            'n_estimators': trial.suggest_int('n_estimators', 10, 200),
            'learning_rate': trial.suggest_float('learning_rate', 1e-2, 1e0, log=True),
            'random_state': self.random_state
    ...
    def _lightgbm_clf_params(self, trial: optuna.Trial) -> Dict[str, Any]:
        ...
        param_dict['objective'] = objective
        
        if objective == 'multiclass':
            param_dict['num_class'] = 3
    ...
    def _lasso_clf_params(self, trial: optuna.Trial) -> Dict[str, Any]:
        return {
            'C': trial.suggest_float('C', 1e-5, 1e2, log=True),
            'penalty': 'l1',
            'solver': 'liblinear',
