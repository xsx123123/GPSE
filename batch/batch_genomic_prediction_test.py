#!/usr/bin/env python3
"""
批量运行基因组预测任务 - 测试版本（软著演示用）
为甜瓜数据集的每个表型依次执行genomic_prediction_v2.py命令
使用预处理功能自动转换TXT表型文件为CSV格式

与正式版本的区别:
  - n_repeats = 1（仅运行1次，快速验证流程）
  - parallel_jobs = 1
  - 结果保存到独立的测试目录
"""
...
RESULTS_BASE_DIR = 'melon_test_results'  # 测试结果目录
PREPROCESS_BASE_DIR = 'melon_test_processed_data'  # 测试预处理数据保存目录
SPECIES_NAME = 'melon'  # 固定物种名称

# genomic_prediction_v2.py 的参数配置（测试版本）
PREDICTION_PARAMS = {
    # 预处理相关参数
    ...
    'top_n_models': 3,
    'n_splits': 5,
    'n_repeats': 1,  # 测试版本：仅运行1次
    'parallel_jobs': 1,  # 测试版本：单线程
    'test_size': 0.3,
    'threads': 1,
    'use_same_test_set': True,
    'save_representative': True,
}
