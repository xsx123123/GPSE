...
- **端到端子命令架构**：`gpse {convert, train, predict, batch, tools, mcp}` 六层子命令贯穿全流程 —— 从原始 `VCF/PLINK` 数据转换、QC 过滤、LD 修剪、基因型-表型匹配，到 `Optuna` 超参优化、多模型评估、`TOPSIS` 排序与 `Stacking` 集成预测，一条命令链完成育种分析交付；
...
- **MCP 智能体原生集成**：内置 [MCP](https://modelcontextprotocol.io) 服务器（`gpse-mcp` / `gpse mcp`），将 `convert → train → predict → batch` 全流程封装为标准 MCP 工具供 AI Agent 直接调用；长时训练自动转入后台任务并支持状态查询、日志拉取与进程停止，实现"对话驱动"的基因组预测分析。
...
* **MCP Server for AI Agents**: Ships an [MCP](https://modelcontextprotocol.io) server (`gpse-mcp` / `gpse mcp`) exposing the full convert → train → predict workflow as MCP tools, with background-job management (status / log / stop) for long-running training — see [§6](#6-mcp-server-gpse-mcp).
...
GPSE ships an [MCP](https://modelcontextprotocol.io) (Model Context Protocol)
server so AI agents can drive the full analysis workflow (convert → train →
predict) through MCP tools. After `pip install`, start it over stdio with
either:

```bash
gpse-mcp        # console script
gpse mcp        # equivalent CLI subcommand
```
...
Exposed tools: `gpse_version`, `gpse_help`, `gpse_convert`, `gpse_train`,
`gpse_predict`, `gpse_batch`, `gpse_tools`, plus background-job management
